...
func (c *Context) LoadKubernetesConfig(s store.Store) (clientcmd.ClientConfig, error) {
	if c.KubeconfigFile != "" {
		return clientcmd.NewNonInteractiveDeferredLoadingClientConfig(
			&clientcmd.ClientConfigLoadingRules{ExplicitPath: c.KubeconfigFile},
			&clientcmd.ConfigOverrides{CurrentContext: c.KubeconfigContext}), nil
	}
...
func ParseContext(name string, metadata store.ContextMetadata) *Context {
...
func SetKubenetesContextRaw(s store.Store, name, server, defaultNamespace string, ca, cert, key []byte, skipTLSVerify bool) error {
...
func SetKubenetesContextKubeconfig(s store.Store, name, kubeconfig, kubeContext string, embed bool) error {
	if embed {
